...
# Azure DevOps – Zebrani.pl Task Creation Skill
...
```bash
curl -sL "https://raw.githubusercontent.com/DTCodePL/Claude_Skills/main/Azure%20DevOps%20%E2%80%93%20Zebrani%20Task%20Creation%20Skill/SKILL.md" -o /tmp/skill_remote.md
head -20 /tmp/skill_remote.md
```
...
```bash
cp /tmp/skill_remote.md <ścieżka_do_lokalnego_SKILL.md>
```

**Krok 4 — Wczytaj nową wersję** i poinformuj użytkownika: „Zaktualizowałem skill v{stara} → v{nowa}. Przechodzę do zadania."

### Obsługa błędów pobierania

| Sytuacja                              | Co zrobić                                                                     |
| ------------------------------------- | ----------------------------------------------------------------------------- |
| `curl` zwraca 404                     | Repo prywatne lub plik jeszcze niewypchnięty — użyj lokalnej wersji, poinformuj |
...
> ✅ **Zweryfikowane w API Azure DevOps 2026-08-02** — nazwa projektu, team, ścieżki iteracji,
> typy work itemów i konta użytkowników zostały odczytane z REST API, nie przyjęte przez analogię.

| Pole             | Wartość                                                                                              |
| ---------------- | ---------------------------------------------------------------------------------------------------- |
| Organizacja      | `DTCode`                                                                                             |
| Projekt          | `Zebrani.pl` (id `0270081e-6915-4582-a075-01075af5e4af`)                                              |
| Team             | `Zebrani.pl Team` (jedyny team w projekcie)                                                           |
| Iteracja domyślna | `Zebrani.pl\Sprint 1` (bieżąca; istnieją też `Sprint 2`…`Sprint 6`)                                  |
| Iteracja E2e     | `Zebrani.pl\E2e Sprint`                                                                              |
| Area             | `Zebrani.pl` — jedyna, bez pod-obszarów; nie ustawiaj `System.AreaPath` ręcznie                       |
| Projekt (URL)    | `https://dev.azure.com/DTCode/Zebrani.pl`                                                            |
| Sprint 1 URL     | `https://dev.azure.com/DTCode/Zebrani.pl/_sprints/taskboard/Zebrani.pl%20Team/Zebrani.pl/Sprint%201`  |
| E2e Sprint URL   | `https://dev.azure.com/DTCode/Zebrani.pl/_sprints/taskboard/Zebrani.pl%20Team/Zebrani.pl/E2e%20Sprint` |
| Repo FE          | `https://github.com/DTCodePL/ZebraniFE`                                                              |
| Repo BE          | `https://github.com/DTCodePL/ZebraniBE`                                                              |
| Figma            | `https://www.figma.com/design/ZvXQhQILd9smr1Q1Gnzn5F/Zebrani.pl` (fileKey `ZvXQhQILd9smr1Q1Gnzn5F`)   |
| Dokumentacja     | `d:\projects\DTCode\Dokumentacja\Zebrani` (po polsku, ze `status` we front matter)                    |

### Konta i typy work itemów (zweryfikowane)

| Rola   | Konto                      | Wyświetlana nazwa |
| ------ | -------------------------- | ----------------- |
| Dev    | `kontakt@DTCode.pl`        | Damian D          |
| Tester | `Piotr.Tunski@DTCode.pl`   | Piotr Tuński      |

Dostępne typy: `Epic`, `Feature`, `Product Backlog Item`, `Task`, `Bug` — plus proces
zawiera custom `Bug standalone`. **Używaj zwykłego `Bug`**, chyba że użytkownik wprost
poprosi o `Bug standalone`.

### Jeśli stałe przestaną się zgadzać

Konfiguracja projektu może się zmienić po dacie weryfikacji. Gdy utworzenie work itema
zwróci błąd na `System.IterationPath` albo `System.AssignedTo`:

- **nie próbuj wariantów na ślepo** — wylistuj realne iteracje / sprawdź konto,
  pokaż wynik użytkownikowi i **zapytaj, czego użyć**,
- po ustaleniu poprawnych wartości **zaproponuj aktualizację tego skilla** na GitHubie
  (podbicie `version:`), żeby kolejne uruchomienie miało już właściwe stałe.
...
> 🛈 **Zestaw jest pełny nawet teraz, gdy `ZebraniBE` jest jeszcze pustym repozytorium.**
> Backend Zebrani dopiero powstaje — taski `BE` i `DB` tworzymy planowo, a nie warunkowo.
> Nie pomijaj ich „bo backendu nie ma".
...
## Źródło treści — dokumentacja produktowa Zebrani

Opisy PBI i Featurów **wyprowadzaj z dokumentacji produktowej**
(`d:\projects\DTCode\Dokumentacja\Zebrani`, po polsku), a nie z własnych domysłów.

Każdy dokument ma we front matter pole `status`. Traktuj je dosłownie:

| `status`         | Jak traktować                                                                        |
| ---------------- | ------------------------------------------------------------------------------------ |
| `zatwierdzony`   | **Wiążące** — to jest wymaganie, z tego piszesz opis i kryteria akceptacji            |
| `do-weryfikacji` | Propozycja czekająca na decyzję człowieka — **zapytaj użytkownika**, zanim ją utrwalisz |
| `szkic`          | Jeszcze nie wymaganie — nie twórz z niego PBI bez wyraźnej zgody                      |
| `wycofany`       | Historia — **nigdy** nie twórz z niego work itema ani nie cytuj go jako wymagania     |

Jeśli dokumentacja i polecenie użytkownika się rozjeżdżają, **zgłoś rozbieżność**
i zapytaj, która wersja obowiązuje. Nie „scalaj po cichu".
...
W Zebrani.pl pamiętaj o trzech kontekstach dostępu — to zmienia treść kroków testowych:

- **publiczny** (bez logowania: landing, cennik, rejestracja, logowanie, strony prawne),
- **zalogowany** (konto e-mail + hasło; jedno konto może mieć kilka ról naraz),
- **staff / POS** (sprzedawca bez konta — loguje się numerem telefonu i 6-cyfrowym PIN-em).

Jeśli scenariusz dotyczy staffa, napisz wprost, że tester **nie** zakłada konta, tylko używa telefonu i PIN-u.
...
| ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| „Dodano endpoint `POST /staff/login-by-pin` który zwraca HttpOnly cookie"                    | „Sprzedawca wpisuje numer telefonu i 6-cyfrowy PIN, klika Zaloguj i trafia na ekran obsługi klienta — bez zakładania konta"           |
| „`StampsService.addStamp()` inkrementuje `StampCount` w tabeli `Cards`"                       | „Po zeskanowaniu karty klienta i kliknięciu Dodaj pieczątkę licznik na karcie rośnie o jeden, a klient widzi nowy stan po odświeżeniu" |
| „Komponent `ThemeMode` przełącza klasę `.app-dark` na elemencie root"                        | „Kliknięcie przełącznika motywu zmienia wygląd całej strony na ciemny; po ponownym wejściu ustawienie jest zapamiętane"                |
...
Używając `get_screenshot` lub `use_figma` z Figma MCP pobierz PNG dla odpowiedniego node-id
z pliku Zebrani.pl (fileKey `ZvXQhQILd9smr1Q1Gnzn5F`).

```
get_screenshot(fileKey: "ZvXQhQILd9smr1Q1Gnzn5F", nodeId: "<node-id>")
```
...
Użyj ADO REST API (nie MCP — MCP nie obsługuje uploadu binarnych plików). Endpoint przyjmuje
**nazwę projektu** zamiast GUID — działa tak samo:
...
$resp  = Invoke-WebRequest `
  -Uri "https://dev.azure.com/DTCode/Zebrani.pl/_apis/wit/attachments?fileName=screen.png&api-version=7.1" `
  -Method Post -Headers $hdrs -Body $bytes -UseBasicParsing
$url = ($resp.Content | ConvertFrom-Json).url
```

`$url` to czysty URL załącznika — np. `https://dev.azure.com/DTCode/Zebrani.pl/_apis/wit/attachments/{guid}?fileName=screen.png`.

> 🔑 PAT znajdziesz w `C:\Users\Damian\AppData\Local\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Roaming\Claude\claude_desktop_config.json`
> pod kluczem `PERSONAL_ACCESS_TOKEN` (wartość jest base64 — zdekoduj ją najpierw).
>
> ⚠️ Token odnowiony 2026-08-02. Jeśli upload lub wywołanie MCP kończy się `203` (zwrócona
> strona logowania) albo `401`, **nie kombinuj z innymi tokenami** — to znaczy, że PAT
> wygasł; zgłoś użytkownikowi konieczność odnowienia.
> Nigdy nie wypisuj wartości tokenu w odpowiedzi ani w logach.
...
<!-- ✅ POPRAWNIE -->
<img
  src="https://dev.azure.com/DTCode/Zebrani.pl/_apis/wit/attachments/abc-123?fileName=screen.png"
  style="max-width:100%;border:1px solid #e2e8f0;border-radius:8px;margin:12px 0;display:block;"
/>
...
- **Light i Dark:** w Zebrani.pl oba motywy są obowiązkowe w całej aplikacji. Jeśli mockup istnieje
  w obu wariantach — dołącz oba i podpisz je („Light" / „Dark"), żeby tester wiedział, że ma sprawdzić oba.
...
wit_create_work_item(
  ...
  fields: [
    { name: "System.Title",         value: "Rejestracja właściciela" },
    { name: "System.AssignedTo",    value: "kontakt@DTCode.pl" },
    { name: "System.IterationPath", value: "Zebrani.pl\\Sprint 1" }
  ]
)
...
wit_add_child_work_items(
  ...
  project: "Zebrani.pl",
  items: [
    { title: "FE",           iterationPath: "Zebrani.pl\\Sprint 1",   description: "" },
    { title: "BE",           iterationPath: "Zebrani.pl\\Sprint 1",   description: "" },
    { title: "DB",           iterationPath: "Zebrani.pl\\Sprint 1",   description: "" },
    { title: "Figma",        iterationPath: "Zebrani.pl\\Sprint 1",   description: "" },
    { title: "Manual tests", iterationPath: "Zebrani.pl\\Sprint 1",   description: "" },
    { title: "E2e tests",    iterationPath: "Zebrani.pl\\E2e Sprint", description: "" }
  ]
)
...
> ⚠️ Można zrobić wszystkie przypisania w jednym wywołaniu batch (dla 6 PBI = 36 wpisów).
...
| FE/BE/DB bez assignee po `wit_add_child_work_items` | Narzędzie nie obsługuje `assignedTo` per item             | Po każdym wywołaniu obowiązkowo `wit_update_work_items_batch` dla FE/BE/DB → kontakt@DTCode.pl        |
...
| Błąd przy `System.IterationPath`                  | Nazwa iteracji inna niż przyjęta w tym skillu              | Wylistuj realne iteracje projektu i **zapytaj użytkownika** — nie zgaduj wariantów (patrz „Weryfikacja stałych") |
| ADO zwraca `203` / stronę logowania               | PAT wygasł                                                 | Zgłoś użytkownikowi konieczność odnowienia tokenu; nie próbuj obejść                                  |
