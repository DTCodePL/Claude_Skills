...
- Organizacja: `DTCode`, projekt: `UstawoŻerca`.
...
- **Git:** commity idą **bezpośrednio na `main`** (zgodnie z historią repo). Konwencja
  wiadomości: `typ(scope): #<id> opis` po polsku (np. `fix(home): #123 ...`).
...
- Figma MCP (jeśli repro odwołuje się do makiety): `get_design_context`,
  `get_variable_defs`, `get_screenshot`. Klucz pliku Figma UstawoŻerca:
  `NF2sMo5czLt2RCVpVHDDnb`.
...
- **MSSQL MCP** (`mcp__mssql__execute_sql`) — dostęp do bazy danych UstawoŻerca. Służy do
  **odczytu danych** przy weryfikacji buga (krok 2) i potwierdzaniu fixa (krok 6) — np.
  sprawdzenie stanu rekordów (użytkownicy, płatności, subskrypcje itp. — w zależności od
  domeny zgłoszenia).
  ...
  - 🛈 Zmiany schematu i tak realizujemy przez **numerowane skrypty SQL**
    (`UstawoZercaBE/Scripts/Script{NNNNN}...`, wymóg CLAUDE.md), a nie przez ten MCP.
...
- Z linku `.../_workitems/edit/<ID>` wyciągnij `<ID>`. Jeśli użytkownik podał sam numer,
  użyj go. Projekt = `UstawoŻerca`.
- `wit_get_work_item(id, project="UstawoŻerca", expand="relations")` — pobierz `System.Title`,
...
- Jeśli zgłoszenie dotyczy wielu bliźniaczych komponentów (np. powtarzalne strony/sekcje),
...
- Dla **każdego** viewportu (`browser_resize` → 360×520, potem 1280×720) powtórz asercje
  (`browser_evaluate`) — teraz muszą zwracać stan oczekiwany (np. poprawny kolor ze zmiennej,
  brak poziomego scrolla). Zrób screenshot „po" na każdym viewporcie do porównania z odpowiednim baseline.
...
- **Ustal numer builda do retestów (FE/BE)** — potrzebny do pierwszego punktu komentarza.
  ...
  - **Repo i workflow deployu:**
    - FE: `DTCodePL/UstawoZercaFE`, workflow `deploy.yml` („Build & Deploy to Production").
    - BE: `DTCodePL/UstawoZercaBE`, workflow `deploy-mikrus.yml` („Deploy to mikr.us").
  - **Jak ustalić numer** (przez `gh`, w shellu z siecią — patrz „Narzędzia"):
    ```
    gh run list --repo DTCodePL/UstawoZercaFE --workflow deploy.yml --limit 1 --json number,headSha,status
    ```
...
- **Obowiązkowo** dodaj komentarz do Buga opisujący **co zostało naprawione** oraz **jak
  ...
  `wit_add_work_item_comment(workItemId=<bug>, project="UstawoŻerca", comment=...)`.
...
- **Kolor** — porównaj computed color do oczekiwanej wartości RGB (kolory w UstawoŻerca
  pochodzą ze zmiennych design-tokenów; odczytaj realnie wyrenderowaną wartość i porównaj):
  ```js
  () => getComputedStyle(document.querySelector('.target-element')).color
  ```
